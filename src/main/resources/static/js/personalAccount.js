...
const divFilterResolvedTaskId = "div-filter-resolved-task";
const divIdFilterRegDate = "div-filter-reg-date";
const divFilterActualTaskId = "div-filter-actual-task";
...
function addEmpContent() {
    const data = getAllObjectsFromRequest(URL_GET_EMPLOYEES + "/" + curUserJson.email);
    const divMainContent = document.getElementById(divIdMainContent);
    ...
    addDataEmployees(divContent, data);
    addDataFilters(divContent);
    setDivUserAtrWidth();
}

function addDataEmployees(divContent, data) {
    ...
    for (let i = 0; i < 2; i++) {
        ...
        selectSearchCriterion.className = "form-control";
    ...
    addListEmployees(divDataEmployees, data.employees);
    addNumberPage(data.pageCount);
}

function addListEmployees(divDataEmployees, employees) {
    ...
    spanCountUsers.textContent = "Всего подчинённых: " + employees.length;
...
function addDataFilters(divContent) {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        switch (i) {
            case 0:
                btn.setAttribute("data-target", "#" + divFilterActualTaskId);
                btn.setAttribute("aria-controls", divFilterActualTaskId);
                spanTitleFilter.textContent = "Актуальных проблем";
                divFilter.id = divFilterActualTaskId;
                break;
            case 1:
                btn.setAttribute("data-target", "#" + divFilterResolvedTaskId);
                btn.setAttribute("aria-controls", divFilterResolvedTaskId);
                divFilter.id = divFilterResolvedTaskId;
...
function addNumberPage(pageCount) {
    ...
    if (pageCount > 9) {
        ...
        for (let i = 0; i < pageCount; i++) {
            ...
            btnPageNumber.textContent = (i + 1).toString();
        ...
        document.querySelectorAll("#" + divPageNumbersId + "> *")
            [selectedPage].className = "btn btn-primary mr-2";
    }
}
